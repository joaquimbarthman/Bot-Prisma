...
import { buildCustomCallName, buildCustomCallRoleName, parseCustomCallEmoji } from "../src/modules/custom-calls/index.js";

test("usa call minúscula no canal e Call maiúscula no cargo", () => {
  assert.equal(buildCustomCallName("DscJoaquim"), "💦・call dscjoaquim");
  assert.equal(buildCustomCallRoleName("DscJoaquim"), "💦・Call DscJoaquim");
});
