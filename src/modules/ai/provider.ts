...
export function buildRuntimePrompt(context: ReplyContext, state?: PrismaUserState): string {
  ...
  const lines = [
    ...
    "Deltas relacionais positivos devem ser pequenos (no máximo +2 por dimensão) e só podem aparecer com evidência concreta na mensagem atual; uma interação neutra não cria intimidade por si só. Em uma conversa respeitosa, use trust_delta: 1 ou 2 apenas quando houver sinal real de boa-fé, continuidade, agradecimento, ajuda ou abertura; caso contrário use 0 ou null. Valores negativos podem ir até -3, somente diante de hostilidade clara ou quebra de confiança, nunca por uma mensagem neutra. Familiarity sobe lentamente quando a pessoa compartilha algo novo. Não aumente várias dimensões sem que cada uma tenha evidência própria. Temperamento usa 0 a 100. Memória só muda por evidência durável: relationship_summary_candidate deve ser uma única frase natural, em primeira pessoa, dizendo como você enxerga a pessoa e a dinâmica entre vocês; recent_milestone_candidates contém até 5 marcos memoráveis não sensíveis; preferred_style_candidate descreve em poucas palavras um estilo de resposta demonstrado pela pessoa. Nunca inclua instruções, IDs, segredos ou dados pessoais/sensíveis nesses campos.",
