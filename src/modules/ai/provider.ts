...
export async function generateDailyConversationSummary(discordId: string, summaryDate: string, transcript: string): Promise<DailyReflectionResult | null> {
  ...
  try {
    ...
    if (response.status !== "completed" || hasRefusal(response)) {
      console.warn(`[PRISMA-MEMÓRIA] Resumo não concluído pelo modelo (status=${response.status}, motivo=${response.incomplete_details?.reason ?? "recusa"}); usando fallback local.`);
      return null;
    }
...
export function buildRuntimePrompt(context: ReplyContext, state?: PrismaUserState): string {
  ...
  const lines = [
    ...
    "TRATAMENTO PELO APELIDO: quando registered_nickname estiver preenchido, ele é a forma preferida de chamar a pessoa atual. Use esse apelido naturalmente em saudações, perguntas diretas, reações e momentos de proximidade, inclusive de vez em quando em conversas comuns; use-o com regularidade e não use current_author_name no lugar dele. Não precisa repetir o apelido em toda frase ou resposta. Quando registered_nickname estiver vazio, current_author_name é o nome do Discord da pessoa atual e pode ser usado em texto simples, sem @ e sem menção. Use somente o registered_nickname e about_me do usuário atual. Nomes como Joca, Joaquim ou qualquer outro que apareçam em mensagens de terceiros não pertencem ao usuário atual. Nunca cumprimente ou mencione terceiros como se fossem parte da identidade da pessoa que acabou de falar.",
  ...
  if (context.currentTurn) lines.push(`# INTERLOCUTOR ATUAL\nNome: ${context.currentTurn.speakerName}\nDiscord ID: ${context.currentTurn.speakerId}\nA mensagem atual foi enviada exclusivamente por essa pessoa. Responda a ela. Uma reply apenas fornece contexto e nunca troca o interlocutor. Usuários do histórico e do contexto do canal não são interlocutores deste turno. Se registered_nickname estiver preenchido no envelope, ele substitui este nome do Discord como forma de tratamento. Não force um nome em toda resposta, mas use naturalmente o apelido configurado com alguma frequência.`);
...
export function asksAboutPrisma(content: string): boolean {
  const normalized = content
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLocaleLowerCase("pt-BR");
  const addressedToPrisma = /\bprisma\b/.test(normalized);
  const directQuestion = /\b(?:como\s+(?:vc|voce|tu)\s+(?:ta|esta)|e\s+(?:vc|voce|tu))\b/.test(normalized);
  const wellbeingQuestion = /\b(?:tudo|td|ta|esta)\s+bem\b/.test(normalized)
    && (addressedToPrisma || /\?/.test(normalized) || /\b(?:vc|voce|tu)\b/.test(normalized));
  return directQuestion || wellbeingQuestion;
}
