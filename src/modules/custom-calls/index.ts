...
async function changeEmoji(interaction: ModalSubmitInteraction, member: GuildMember): Promise<void> {
  ...
  const channelName = buildCustomCallName(member.user.username, emoji);
  const roleName = buildCustomCallRoleName(member.user.username, emoji);
  await channel.setName(channelName, `Emoji da call personalizada alterado por ${member.id}`);
  await role.setName(roleName, `Emoji da call personalizada alterado por ${member.id}`);
