...
function draftView(draft: Draft): { components: APIContainerComponent[] } {
  ...
  const options = new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`${PREFIX}draft-voice`).setLabel("Call automática").setEmoji(lfgSoundEmoji() ?? "🎵").setStyle(draft.autoVoiceEnabled ? ButtonStyle.Success : ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`${PREFIX}draft-note`).setLabel("Adicionar observação").setEmoji(lfgWarningEmoji() ?? "⚠️").setStyle(ButtonStyle.Secondary));
...
async function showCreate(interaction: ButtonInteraction): Promise<void> {
  const last = createdCooldowns.get(interaction.user.id) ?? 0; if (Date.now() - last < config.lfg.createCooldownSeconds * 1000) { await interaction.reply({ content: "Aguarde um instante antes de criar outro LFG.", flags: ["Ephemeral"] }); return; }
...
async function submitCreate(interaction: ButtonInteraction, draft: Draft): Promise<void> {
  ...
  if (!interaction.channelId) { await interaction.reply({ content: "Não encontrei o canal para publicar este LFG.", flags: ["Ephemeral"] }); return; }
...
export async function handleLfgInteraction(interaction: Interaction): Promise<boolean> {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === `${PREFIX}game`) { const draft: Draft = { game: interaction.values[0] as LfgGameKey, maxPlayers: 4, autoVoiceEnabled: false, note: "", expiresAt: Date.now() + 15 * 60_000 }; drafts.set(draftKey(interaction), draft); await interaction.update(draftView(draft)); return true; }
  ...
  if (action.startsWith("draft-")) { const draft = drafts.get(draftKey(interaction)); if (!draft || draft.expiresAt < Date.now()) { await interaction.reply({ content: "Essa criação expirou. Comece novamente.", flags: ["Ephemeral"] }); return true; } if (action === "draft-voice") { draft.autoVoiceEnabled = !draft.autoVoiceEnabled; await interaction.update(draftView(draft)); return true; } if (action === "draft-note") { await interaction.showModal({ customId: `${PREFIX}draft-note-modal`, title: "Observação do grupo", components: [new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder().setCustomId("note").setLabel("Modo, rank ou objetivo (opcional)").setStyle(TextInputStyle.Paragraph).setValue(draft.note).setRequired(false).setMaxLength(500))] }); return true; } if (action === "draft-cancel") { drafts.delete(draftKey(interaction)); await interaction.update({ components: [{ type: ComponentType.Container, accent_color: 0x99aab5, components: [{ type: ComponentType.TextDisplay, content: "Criação cancelada." }] }] }); return true; } if (action === "draft-create") { await submitCreate(interaction, draft); return true; } }
  const session = (await sessions()).find((value) => value.id === id); if (!session) { await interaction.reply({ content: "Este LFG não existe mais.", flags: ["Ephemeral"] }); return true; }
