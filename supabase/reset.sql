...
drop function if exists public.complete_prisma_daily_summary(text, date, text, text[]);
drop function if exists public.claim_prisma_daily_summary(text, date, integer);
...
drop table if exists public.prisma_daily_summary_jobs cascade;
...
drop table if exists public.prisma_self_learning_evidence cascade;
